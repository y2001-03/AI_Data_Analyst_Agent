...
AI Data Analyst Agent 是一个面向演示与原型验证的数据分析智能体项目。系统支持上传 CSV / XLSX 数据文件，并通过自然语言驱动分析流程，自动完成数据理解、任务规划、工具执行、结果生成和调试追踪展示。
...
flowchart TD
    ...
    understand --> memory[Session Memory Service]
...
上面的 Mermaid 代码使用 `flowchart TD`，并避免将 `graph` 用作节点 id，可在 GitHub 中正常渲染。
...
```bash
cd D:\vscode\AI_Data_Analyst_Agent
...
- DuckDB SQL Tool：已完成，支持基于上传数据集的 SQL 分析
- Session Memory / Dataset Context：已完成，支持同一数据集下连续追问
- Debug / Trace Observability：已完成
- Frontend DebugPanel：已完成

当前项目适合作为 V1 演示版本，重点突出：
